...
export const SlotsManagement: React.FC<SlotsManagementProps> = ({ readOnly = false }) => {
    ...
    // 🚀 FORCER LA DISPONIBILITÉ (override délai 2h) - GLOBAL
    const handleForceAvailableGlobal = async (slotId: string) => {
        const confirmed = confirm('⚠️ Attention : Forcer la disponibilité GLOBALEMENT permet à TOUS les magasins de réserver ce créneau même si le délai de 2h n\'est pas respecté.\n\nConfirmer ?');
        if (!confirmed) return;

        try {
            await slotsService.forceSlotAvailable(selectedDate, slotId, undefined);
            await loadAvailability();
            await loadRestrictions();
        } catch (error) {
            console.error('Erreur forcer disponibilité globale:', error);
            alert('Erreur lors du forçage de disponibilité globale');
        }
    };

    // 🚀 FORCER LA DISPONIBILITÉ (override délai 2h) - POUR UN MAGASIN
    const handleForceAvailableMagasin = async (slotId: string) => {
        if (!selectedMagasinId) {
            alert('⚠️ Veuillez sélectionner un magasin spécifique');
            return;
        }

        const magasinName = magasins.find(m => m.id === selectedMagasinId)?.nom || 'ce magasin';
        const confirmed = confirm(`⚠️ Attention : Forcer la disponibilité UNIQUEMENT pour "${magasinName}".\n\nCe créneau sera disponible seulement pour ce magasin.\n\nConfirmer ?`);
        if (!confirmed) return;

        try {
            await slotsService.forceSlotAvailable(selectedDate, slotId, selectedMagasinId);
            await loadAvailability();
            await loadRestrictions();
        } catch (error) {
            console.error('Erreur forcer disponibilité magasin:', error);
            alert('Erreur lors du forçage de disponibilité pour le magasin');
        }
    };

    const handleRemoveForce = async (slotId: string) => {
        const confirmed = confirm('Retirer le forçage de disponibilité pour ce créneau ?');
        if (!confirmed) return;
        ...
        try {
            ...
            console.error('Erreur retrait forçage:', error);
            alert('Erreur lors du retrait du forçage');
        }
    };
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="text-sm text-gray-600 mb-3">
                                        ...
                                        {slot.forceAvailable && (
                                            <p className="text-orange-600 font-medium text-xs">⚡ Forcé disponible par admin</p>
                                        )}
                                    ...
                                    {!readOnly && (
                                        <div className="flex flex-col space-y-2">
                                            {/* Ligne 1 : Bloquer/Débloquer/Retirer forçage */}
                                            <div className="flex space-x-2">
                                                {blocked ? (
                                                    ...
                                                    <button
                                                        onClick={() => handleRemoveForce(slot.slot.id)}
                                            ...
                                            {/* Ligne 2 : Boutons Forcer disponible (uniquement pour le délai 2h, pas pour dates/créneaux passés) */}
                                            {!blocked && !slot.forceAvailable && slot.reason === 'Créneau passé (délai de prévenance 2h)' && (
                                                <div className="flex space-x-1">
                                                    <button
                                                        onClick={() => handleForceAvailableGlobal(slot.slot.id)}
                                                        className="flex-1 px-2 py-1 bg-orange-600 text-white text-xs rounded hover:bg-orange-700 font-medium"
                                                        title="Forcer disponible pour tous les magasins"
                                                    >
                                                        🌍 Forcer global
                                                    </button>
                                                    {selectedMagasinId && (
                                                        <button
                                                            onClick={() => handleForceAvailableMagasin(slot.slot.id)}
                                                            className="flex-1 px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700 font-medium"
                                                            title={`Forcer disponible uniquement pour ${magasins.find(m => m.id === selectedMagasinId)?.nom}`}
                                                        >
                                                            🏪 Forcer magasin
                                                        </button>
                                                    )}
