...
export const OfflineProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [currentDataSource, setCurrentDataSource] = useState<DataSource>(DataSource.AUTO);

    // Initialiser l'adaptateur de données
    const [dataService] = useState(() => {
        const adapter = new DataServiceAdapter(
            import.meta.env.VITE_AIRTABLE_TOKEN,
            DataSource.AUTO
        );

        // Configurer le mode hors ligne si nécessaire
        if (isOfflineForced) {
            adapter.setForcedOfflineMode(true);
        }
    ...
    const synchronize = useCallback(async (): Promise<boolean> => {
        if (!isOnline || isSynchronizing) {
            console.log("Synchronisation impossible: mode hors ligne ou déjà en cours");
            return false;
        }

        try {
            setIsSynchronizing(true);
            console.log('[OfflineContext] Début de la synchronisation...');
            ...
            if (success) {
                ...
                // Recompter les changements après synchronisation
                const count = await db.pendingChanges.count();
                setPendingChangesCount(count);

                console.log(`[OfflineContext] Synchronisation réussie. ${count} changements restants.`);
                return true;
            } else {
                console.log("[OfflineContext] Échec de la synchronisation");
                return false;
            }
        } catch (error) {
            if (!handleStorageError(error)) {
                console.error("[OfflineContext] Erreur lors de la synchronisation:", error);
            }
            return false;
        } finally {
            setIsSynchronizing(false);
        }
    }, [isOnline, isSynchronizing, dataService]);
